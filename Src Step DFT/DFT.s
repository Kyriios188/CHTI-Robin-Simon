...
masque equ 0x3f ;masque valant 63
; ===============================================================================================
	extern LeSignal 
...
boucle
	...
	ldr r3, =masque
	mul r7, r2, r1
	and r7, r7, r3 ;modulo(63) via un et logique et un masque

	ldrsh r4, [r0, r2, lsl #1] ;récupérationde la portion de signal, format 1.12
	
	ldr r3, =TabCos ;format 1.15
	ldrsh r3, [r3, r7, lsl #1] ;le cosinus
	mul r3, r4 ;format 2.27
	asr r3, #7 ;format 2.20
	
	add r5, r3 ;somme pour les cosinus
	
	ldr r3, =TabSin ;format 1.15
	ldrsh r3, [r3, r7, lsl #1] ;le sinus
	mul r3, r4 ;format 2.27
	asr r3, #7 ;2.20
	
	add r6, r3 ;somme pour les sinus

	add r2, #1
	cmp r2, #64
	bne boucle
	
	asr r5, #12 ;format 8.8
	asr r6, #12 ;format 8.8
	mul r5, r5 ;format 16.16
	mul r6, r6 ;format 16.16
	asr r6, #1
	asr r5, #1
	add r0, r5, r6 ;format 17.15
